"""milpa scan — análisis automático y extensible de la app (estilo milpa).
...
(eager = cargado / lazy = diferido con milpa.lazy / absent = solo dentro de funciones). Luego
...
class ModuleInfo:
    ...
    lazy_imports: list[str] = field(default_factory=list)  # libs vía `from milpa.lazy import X` (✓)
...
def _top_level_imports(tree: ast.Module) -> tuple[set[str], set[str]]:
    ...
    - lazied = libs traídas vía `from milpa.lazy import X` (el patrón bueno — diferidas)."""
    eager: set[str] = set()
    lazied: set[str] = set()
    for node in tree.body:  # solo top-level
        if isinstance(node, ast.Import):
            ...
            if node.module in _LAZY_MODULES:  # from milpa.lazy import openpyxl, pandas
...
def capability(cls: type[Capability]) -> type[Capability]:
    ...
    capacidades propias, así que registrarse al importar basta (estilo milpa)."""
                ...
                if lib in _HEAVY:
                    out.append(
                        Finding(
                            ...
                            hint=f"Si este proceso no siempre la usa: `from milpa.lazy import {lib}`.",
                        )
                    )
            for lib in m.lazy_imports:  # ya vía milpa.lazy (el patrón bueno)
                if lib in _HEAVY:
                    out.append(
                        Finding(
                            "lazy",
                            "ok",
                            m.dotted,
                            f"'{lib}' ya está diferida con milpa.lazy ✓ — no pesa donde no se usa.",
                        )
                    )
        return out
        ...
        for m in model.modules:
            for lib in m.top_imports:
                if lib in _HTTP:
                    ...
                    extra = " (además EAGER: difiérelo con milpa.lazy si no se usa siempre)" if eager else ""
                    out.append(
                        Finding(
                            "http",
                            "info",
...
class _Db(Capability):
    """Engine/session de SQLAlchemy armados a mano — saltan la sesión ambiente de milpa."""
            ...
            if used:
                out.append(
                    Finding(
                        ...
                        hint="Usa la sesión ambiente de milpa (milpa.Core.Database / @transactional).",
                    )
                )
        return out
                ...
                if lib in {"pymongo", "motor"}:
                    out.append(
                        Finding(
                            ...
                            hint="Aísla la conexión: lazy + config por .env (patrón provider estilo milpa).",
                        )
                    )
        return out
        ...
        if not getattr(settings, "content_security_policy", ""):
            out.append(
                Finding(
                    ...
                    hint="milpa trae CSP report-only por default; defínelo (no lo vacíes).",
                )
            )
...
def format_report(findings: list[Finding]) -> str:
    ...
    lines.append(f"\n  Resumen: {warn} a revisar · {info} sugerencias · {ok} ya en estilo milpa")
