"""Import perezoso (estilo milpa): difiere libs pesadas/opcionales al PRIMER uso.
...
def load(name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise ModuleNotFoundError(f"milpa.lazy: no se encontró el módulo '{name}'")
...
def __getattr__(name: str) -> ModuleType:  # PEP 562
    """`from milpa.lazy import openpyxl` cae aquí -> openpyxl perezoso."""
    # Los dunders (__path__, __all__, ...) NO son libs: deja que Python los maneje normal
    # (sin esto, `from milpa.lazy import X` intenta cargar el módulo '__path__' y truena).
