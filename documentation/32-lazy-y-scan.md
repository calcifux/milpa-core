# Imports perezosos (`milpa.lazy`) y análisis (`jornal scan`)

## El principio (el camino milpa)

milpa **importa todos los módulos de tu app al arrancar** para registrarlos solos: tasks,
...
pagando RAM (y arranque) donde no hace falta. El camino milpa: **el discovery registra todo,
pero tú difieres lo pesado que cada proceso no siempre usa.**

## `milpa.lazy` — difiere la lib hasta el primer uso

```python
from milpa.lazy import openpyxl          # NO carga openpyxl aún — solo lo nombra
...
no rompe nada, solo no ahorra. Y **genérico**: cualquier lib (`from milpa.lazy import pandas`,
...
|---|---|
| `lazy` | libs pesadas (openpyxl, pandas, numpy, torch...) importadas **eager** → sugiere `milpa.lazy`. Acredita ✓ las que ya difieres. |
...
```python
from milpa.scan import capability, Capability, Finding
...
milpa — la convención vive como conocimiento ejecutable, auto-descubrible, que escala a N apps.
