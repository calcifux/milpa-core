"""Tests de milpa.scan: parseo del modelo + capacidades (lógica pura, sin discovery real)."""
...
from milpa import scan as scan_mod
from milpa.scan import AppModel, Finding, ModuleInfo, _Db, _Http, _Lazy, capability_names
...
def test_top_level_imports_separates_eager_lazy_and_ignores_nested() -> None:
    ...
    assert "numpy" in lazied  # 'from milpa.lazy import X' -> es el patrón bueno
...
def test_third_party_filter_excludes_stdlib_and_own() -> None:
    ...
    assert eager == {"openpyxl"}  # os (stdlib), milpa/app (propios) NO cuentan
...
def test_lazy_capability_credits_milpa_lazy_usage() -> None:
