...
from milpa.Core.Console.Cli import _render_cli_error
...
def test_domain_error_renders_clean_without_traceback(capsys: CaptureFixture[str]) -> None:
    code = _render_cli_error(ForbiddenError("No autorizado para esta acción."))

    assert code == 1
    out = capsys.readouterr().out
    assert "No autorizado para esta acción." in out  # el mensaje del dominio, legible
    assert "forbidden" in out  # el código estable, como pista
